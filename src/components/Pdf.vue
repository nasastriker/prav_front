    ...
    <div class="container-fluid">
        <div id="PdfHeader">
            <div class="mainbox">
                <div class="title withoutline">
                    <span class="right" id="PageInfo" v-if="!error">Страницы {{current}}/{{totalPage}}</span>
                    <span class="right mr-3" v-if="!error">
						<button class="btn btn-sm btn-success mr-2" id="prev-page" @click="RenderPage(current-1)" :disabled="current <= 1"><i class="fas fa-arrow-circle-left mr-1"></i> Пред.</button>
						<button class="btn btn-sm btn-success" id="next-page" @click="RenderPage(current+1)" :disabled="totalPage<=current">След.<i class="fas fa-arrow-circle-right ml-1"></i></button>
					</span>
                    <span class="right mr-3" v-if="!error">
        ...
        <div class="mainbox" id="ContentArea">
            ...
            <div class="content scroll text-center" style="">
                <div class="alert alert-danger text-left" v-if="error">Не удалось открыть. </div>
