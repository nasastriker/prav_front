...
<template>
    <div>
      ...
      {'disabled-item pointer-events-none' : isDisabled}
            ...
            <a v-else :target="target" :href="href" tabindex="-1" style="padding-bottom: 0px;padding-top: 0px">
                <vs-icon v-if="!featherIcon" :icon-pack="iconPack" :icon="icon" />
                <feather-icon v-else :class="{'w-3 h-3': iconSmall}" :icon="icon" />
                <slot />
            </a>
...
export default {
  ...
  computed: {
    canSee () {
      ...
      let res =this.to ? this.$acl.check(this.$router.match(this.to).meta.rule) : true
      return res
    },
    activeLink () {

      return !!((this.to === this.$route.path ||this.$route.path.startsWith(this.to+'/') ||this.$route.meta.parent === this.slug) && this.to)
    }
  }
}
