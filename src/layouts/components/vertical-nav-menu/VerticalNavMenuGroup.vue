  ...
  <div
    ...
    @mouseout  = "mouseout">
      ...
      <ul ref="items" :style="styleItems" class="vs-sidebar-group-items">
        <li v-for="(groupItem, index) in group.submenu" :key="index" v-if="openItems">
          <!-- If item is group -->
          <v-nav-menu-group
        ...
        const func = (item) => {
          if (item.submenu) {
            ...
            item.submenu.forEach((item) => {
              if ((path === item.url ||active|| routeParent === item.slug) ) { open = true } else if (item.submenu) { func(item) }
            })
          }
        }
