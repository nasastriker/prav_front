      ...
      <div @mouseenter="mouseEnter" @mouseleave="mouseLeave">
        ...
        <div class="header-sidebar flex items-end justify-between" slot="header">
          ...
          <router-link tag="div" class="vx-logo cursor-pointer flex items-center" to="/">
            ...
            <span class="vx-logo-text text-primary" v-show="isMouseEnter || !reduce" v-if="title">BV-Pravezh</span>
          </router-link>
          <!-- /Logo -->
        ...
        <div style="text-align: right;color: #1f2b7b;margin-bottom: 15px;padding-right: 20px">
          <span style="font-size: 8pt;">Версия: <b>№01072023</b></span>
        </div>
        <!-- /Header -->
        ...
        <div class="vx-row center place-content-center">
          <div style="margin: 0 auto; padding: 0!important;">
            <vs-button class="mb-6 mt-4 place-content-center" icon="logout" color="primary" style="width: 100%; display:flex;" @click="logout">
              Выход
            </vs-button>
          </div>
        </div>
...
export default {
            ...
            item.submenu.forEach((item) => {
              ...
              if(item.submenu){
                for (let i = 0; i < item.submenu.length; i++) {
                  if (path.startsWith(item.submenu[i].url+'/')){
                    active=true
                  }
                }
              ...
              if ( (path === item.url ||active|| routeParent === item.slug|| path.startsWith(item.url+'/'))) { open = true } else if (item.submenu) { func(item) }
            })
          }
        }
       // console.log(item)
  ...
  methods: {
    ...
    logout(){
      axios.post('/api/log/logout').then((response) => {
        this.$router.push('/')
