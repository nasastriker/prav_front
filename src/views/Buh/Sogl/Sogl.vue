        ...
        <div class="vx-card p-6">
            <div class="flex flex-wrap justify-between items-center">
                ...
                <div class="flex flex-wrap items-center justify-between ag-grid-table-actions-right">
                    <vs-input class="mb-4 md:mb-0 mr-4" v-model="searchQuery" @input="updateSearchQuery"
                              placeholder="Поиск..."/>
                    <vs-button color="success" class="pull-right" type="filled" @click="$router.push('/buh/sogl/new')">
                        Новое
                    </vs-button>
            ...
            <vs-popup classContent="popup-example" title="Соглашение о рассрочке" :active.sync="popupActive3">
                ...
                <div class="vx-row">
                    <div class="vx-col">
                        <h6>Статус:</h6>
                        <vs-input class="w-100 mb-base" style="margin-left: 0px;" v-model="sogl.status"/>
                    </div>
                </div>
                <div class="vx-row">
                    ...
                    <div class="vx-col w-full">
                        <h6>ФИО:</h6>
                        <vs-input class="w-full mb-base" v-model="sogl.fio"/>
                    </div>

                </div>
                <div class="vx-row">
                    <div class="vx-col md:w-1/2 w-full mt-5">
                        <h6>Дата первого платежа:</h6>
                        <vs-input type="date" class="w-100 mb-base" v-model="sogl.date1"/>
                    </div>
                    <div class="vx-col md:w-1/2 w-full mt-5">
                        <h6>Дата второго платежа:</h6>
                        <vs-input type="date" class="w-100 mb-base" v-model="sogl.date2"/>
                    </div>
                    <div class="vx-col md:w-1/2 w-full mt-5">
                        <h6>Сумма первого платежа:</h6>
                        <vs-input type="number" class="w-100" v-model="sogl.sum1"/>
                    </div>
                    <div class="vx-col md:w-1/2 w-full mt-5">
                        <h6>Сумма второго платежа:</h6>
                        <vs-input type="number" class="w-100" v-model="sogl.sum2"/>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        onrowDoubleClicked(event) {
            ...
            this.sogl.date1 = event.data.date1;
            this.sogl.date2 = event.data.date2;
            this.sogl.sum1 = event.data.sum1;
            this.sogl.sum2 = event.data.sum2;
            this.sogl.fio = event.data.fio;
            if (event.data.status == 1) {
                this.sogl.status = 'Заключено'
            } else {
                this.sogl.status = 'Не заключено'
            }
            this.popupActive3 = true;
            //
        },
