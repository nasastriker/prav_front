<template>
    <div class="excel-import" style="margin-left: 20px">
        ...
        <div style="margin-bottom: 10px">
            <a v-auth-href  href="/example_file/?filename=type_correspondence_filter_sample" >Образец </a>
        </div>
        <div style="margin-bottom: 10px">
            <a v-auth-href  href="/example_file/?filename=type_correspondence_filter_ident" >Список идентификаторов</a>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions([
                'checkCorrespondenceFilterFromXLS','exportCorrespondenceToXLS'
            ]),
            ...
            generateData ({ header, results, meta,name }) {
                ...
                if(typeof this.excelData.header==='object'&&
                    ...
                    this.checkCorrespondenceFilterFromXLS({header:this.excelData.header,data:this.excelData.results}).then(res=>{
                        this.textError=res.error||''
                        console.log(res.error)
                        this.textCheck=(typeof res.error==='string')?Boolean(res.error.length):false
                        if(!this.textCheck){
                            this.$emit('closePopup')
                            this.exportCorrespondenceToXLS({header:this.excelData.header,data:this.excelData.results,name:this.excelData.name,user:this.User.name_family+' '+this.User.name})
                            this.$vs.notify({
                                title: 'Запуск',
