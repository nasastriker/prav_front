<template>
  <div>
    ...
    <template v-else-if="(params.type_f=='list_users')">
      <Select2 v-model="listId" :modelValue="listId" :options="AllUsersForAutoTaskSets"
               :placeholder="params.placeholder" style="margin-left: -44px;width: 431px;" :settings="{ width: '431'}"
               @select="mySelectEvent($event)"/>
    </template>
    <template v-else>
      <vs-input v-model="textFilter" style="margin-left: -30px;width: 440px;" :placeholder="params.placeholder" @blur="onInput"/>
    </template>
  </div>
</template>
