<template>
    <div>
     <template v-if="(params.type_f=='date')">
         <vs-input  type="date" style="max-width: 130px;margin-left: -25px" v-model="dateBegin"  @blur="onBlur"/>
      </template>
      <template v-else-if="(params.type_f=='list_status_task')">
        <Select2 v-model="listIdStat" :options="AutoTaskSetsTasksStatusList" style="margin-left: -25px" :settings="{ width: '125'}" @select="mySelectEvent($event)" />
      </template>
       <template v-else>
