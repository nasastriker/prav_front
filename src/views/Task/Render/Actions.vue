<template>
    <div>
        ...
        <div v-if="params.is_admin === 1" title="Удалить">
            <div v-if="params.data.status === 2">
                <ArchiveIcon size="1.5x" class="hover:text-primary cursor-pointer" @click="SendToArchive"></ArchiveIcon>
                <trash-2-icon size="1.5x" title="Удалить" class="hover:text-primary cursor-pointer" @click="confirmDeleteRecord"></trash-2-icon>
            </div>
            <div v-else>
                <trash-2-icon size="1.5x" title="Удалить" class="hover:text-primary cursor-pointer" @click="confirmDeleteRecord"></trash-2-icon>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions([
                'deleteTaskUserOnes', 'doneTaskUser','SendDoneTaskOnesToArchive','getDataTasksUserOnes'
            ]),
            SendToArchive(){
            ...
            deleteRecord() {
                this.deleteTaskUserOnes(this.params.data.id).then((value)=> {
                    if(value){
