  ...
  methods: {
    onGridReady() {
      this.gridOptions.api.forEachNode(node=>
          node.data.id == this.selectId ? node.setSelected (true) : node.setSelected(false));
    },
    onSelectionChanged() {
    ...
    changeCheck(e){
      ...
      this.lastKnownTopPixel = this.gridApi.getVerticalPixelRange().top
      this.selectId=this.gridApi.getSelectedRows()[0].id
      if(this.AnsCreditsArrLo.length==0) {
