            ...
            <div class="vx-col w-1/3">
                <vs-dropdown vs-trigger-click class="cursor-pointer">
                    <div class="cursor-pointer flex items-center justify-between font-medium " style="padding: 0.75rem !important;margin-top: 0px;border: 1px solid #ccc;border-radius: 4px;">
                        <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ TotalSudArr - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : TotalSudArr }} of {{ TotalSudArr }}</span>
...
import TaskSudStatus from './TaskSudStatus.vue'
...
export default {
        ...
        return {
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    filter: true,
                    width: 70,
                    cellRendererFramework: 'TaskSudStatus'
                },
                {
                    headerName: 'Ошибка',
    ...
    computed: {
        ...mapGetters([
            'TaskSudArr','User','TotalSudArr','ArchSudsArr'
        ]),
        totalPages () {
            if (this.gridApi)
                return Math.ceil(this.TotalSudArr/this.paginationPageSize)
            else return 0
        },
        paginationPageSize () {
            return this.User.pag.taskSudHistory.limit
        },
        ...
        currentPage: {
            ...
            set (val) {
                this.setTaskSudHistoryOffset(val-1)
    ...
    methods: {
        ...mapActions([
            'getTaskSuds','setDataUser','getDataUser'
        ]),
        ...mapMutations([
            'setTaskSudHistoryLimit','setTaskSudHistoryOffset',
        ]),
        reload(e){
          if(this.taskShow){
            this.getTaskSuds(this.User.pag.taskSudHistory);
        ...
        changeStatus(){
          this.User.pag.taskSudHistory.bankTaskSudStatus=this.status
          this.setDataUser().then((response) => {
            this.getTaskSuds(this.User.pag.taskSudHistory);
          })
        },
        changePag(pag){
            this.User.pag.taskSudHistory.limit=pag
            this.setTaskSudHistoryLimit(pag);
            this.setDataUser()
            this.getTaskSuds(this.User.pag.taskSudHistory);
        ...
        changeDate(){
            this.User.pag.taskSudHistory.bankTaskSudDate=this.bankTaskSudDate
            this.setDataUser().then((response) => {
                this.getTaskSuds(this.User.pag.taskSudHistory);
            })
        },
    ...
    mounted() {
        ...
        this.getDataUser().then(res=>{
            this.bankTaskSudDate=this.User.pag.taskSudHistory.bankTaskSudDate||''
            this.User.pag.taskSudHistory.offset=0
            this.getTaskSuds(this.User.pag.taskSudHistory);
