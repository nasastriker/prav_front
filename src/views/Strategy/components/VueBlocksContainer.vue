...
<script>
  ...
  import mouseHelper from '../src/helpers/mouse'
...
//          })
          ...
          if (this.linkStartData.block.id !== targetBlock.id) {
            let add_link_flag = true;
            let error_text = '';

            if (this.linkStartData.block.type == targetBlock.type){
              if (targetBlock.type=='status' && this.linkStartData.block.type=='status'){
                add_link_flag = false;
                error_text = 'Статус со статусом не может быть связан без функции';
              }
            }

             // if(this.linkStartData.block.type!=targetBlock.type||targetBlock.type=='func'&&this.linkStartData.block.type=='func'){
            if (add_link_flag){
                  this.links.push({
                      id: maxID + 1,
      ...
      // Blocks
        addNewBlockFunc (nodeName,id_func,first, x, y) {
      ...
      addNewBlockStatus (nodeName,id_status, x, y) {
          let maxID = Math.max(0, ...this.blocks.map(function (o) {
              return o.id
          }))
      ...
      addNewBlockCond (nodeName,cond, x, y) {
        let maxID = Math.max(0, ...this.blocks.map(function (o) {
          return o.id
        }))
