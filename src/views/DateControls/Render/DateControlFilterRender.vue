<template>
    <div>
     ...
     <template v-if="(params.type_f=='date')">
         <vs-input  type="date" style="margin-left: -30px;max-width: 150px" v-model="dateBegin"  @blur="onBlur"/>
      </template>
      <template v-else-if="(params.type_f=='list')">
        <Select2 v-model="listId" :options="DateControlTypes" style="margin-left: -22px" :settings="{ width: '300'}" @select="mySelectEvent($event)" />
      </template>
       <template v-else>
       <vs-input  v-model="textFilter" style="margin-left: -30px;min-width: 400px" @blur="onInput"/>
      </template>
