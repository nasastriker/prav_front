...
export default {
  name: 'OpenDateControlTaskStatus',
  mounted() {
  ...
  computed: {
    ...
    statusName() {
      return (value) => {
        ...
        if (value == 1) return 'Формируется'
        if (value == 2) return 'Ожидает'
        if (value == 3) return 'Работает'
        if (value == 4) return 'Выполнена'
        if (value == 5) return 'Ошибка'
      }
    },
    chipColor() {
      return (value) => {
