        ...
        <div class="vx-card p-6">
            <div class="vx-row" style="z-index: 222; position: relative;">
                ...
                <div style="margin-right: 10px; min-width: 250px;">
                    ...
                    <v-select  class="w-50" :reduce="label => label.id" label="name" :options="RecoverArrList"  v-model="rec_id" @input="setRecover" ></v-select>
                </div>
                ...
                <div style="margin: 28px 20px 0;">
                    <div class="vx-row">
                        <vs-tooltip text="Ускоренный поиск" position="top" >
                            <vs-checkbox v-model="User.pag.credit.fast" @input="updateSearchFast" ></vs-checkbox>
                        </vs-tooltip>
                    </div>
                </div>
                <!-- <div class="vx-col" v-if="CreditsFindFlag">
                    <div class="vx-row" >
                        <span style="max-width: 40px;">
                            <img src="loading.gif" style="max-width: 40px; margin-top: 20px;">
                        </span>
                    </div>
                </div> -->

                <div style="margin-right: 10px; margin-top: 17px;">
                ...
                <div style="margin-right: 10px; margin-top: 17px;">
                    <vs-button @click="filterReset" class="mr-4">Сбросить фильтры</vs-button>
                    <vs-button class="includeIconOnly" @click="addDebshow=!addDebshow">+</vs-button>
                </div>
                <div style="display: flex">
                <div class="settingsBtnWrapper" style="margin-right: 10px;margin-top: 17px;">
                    <div class="centerx">
                        <vs-tooltip text="Настройки таблицы" position="top" >
                            <vs-button class="btn-drop settingsBtn includeIconOnly"  @click="tableControl=true">
                                <feather-icon icon="SettingsIcon" class="cursor-pointer" style="height: 18px; width: auto"></feather-icon>
                            </vs-button>
                        </vs-tooltip>
                    </div>
                </div>
                <div class="settingsBtnWrapper" style="margin-top: 17px;">
                    <div class="centerx">
                        <vs-tooltip text="Скачать" position="top" >
                            <vs-button class="btn-drop downloadBtn includeIconOnly"  @click="startDownload">
                                <feather-icon icon="DownloadIcon" class="cursor-pointer" style="height: 18px; width: auto"></feather-icon>
                            </vs-button>
                        </vs-tooltip>
                    </div>
                </div>
            ...
            <vs-popup classContent="popup-example" title="Добавить заемщика" :active.sync="addDebshow">
                <div class="vx-row">
                    <div class="vx-col md:w-1/3 w-full">
                        <h6 class="h6" style="margin-top: 25px">Фамилия:</h6>
                        <vs-input class="w-100 "  v-model="newDeb.debtor.name_family"></vs-input>
                        <h6 class="h6">Имя:</h6>
                        <vs-input class="w-100 " v-model="newDeb.debtor.name" ></vs-input>
                        <h6 class="h6">Отчество:</h6>
                        <vs-input class="w-100 "  v-model="newDeb.debtor.name_patronymic" ></vs-input>
                        <h6 class="h6">Дата рождения:</h6>
                        <vs-input type="date" class="w-100 " v-model="newDeb.debtor.birthdate"></vs-input>
                        <h6 class="h6" style="margin-top: 15px">Номер договора:</h6>
                        <vs-input class="w-100"  v-model="newDeb.debtorCredit.number_dog" ></vs-input>
                        <h6 class="h6">Дата договора:</h6>
                        <vs-input type="date" class="w-100" v-model="newDeb.debtorCredit.date_dog"></vs-input>
                    </div>
                    <div class="vx-col md:w-2/3 w-full">
                        <h6 class="h6" style="margin-left:20px;margin-top: 25px">Взыскатель или договор цессии:</h6>

                        <v-select  class="w-50 " style="margin-bottom: 10px;margin-left:20px;min-width: 350px" :reduce="label => label.id" label="name"
                                   :options="optArr"  v-model="newDeb.debtorCredit.id_recover"  ></v-select>
                        <v-select  class="w-50 " style="margin-bottom: 10px;margin-left:20px;min-width: 350px" :reduce="label => label.id" label="name"
                                   :options="StatussArr"  v-model="newDeb.debtorCredit.id_status"  ></v-select>
                ...
                <div style="text-align: center;margin-top: 10px">
                    <vs-button @click="addDebtor">Добавить</vs-button>
                </div>
            </vs-popup>
    ...
    export default {
        ...
        data () {
            return {
                ...
                columnDefs: [
                    ...
                    {
                        headerName: '№ СА',
                        headerTooltip: 'Номер судебного акта',
                  ...
                  {
                    headerName: '№ дела иск',
                    headerTooltip: '№ дела иск',
                    tooltipField: 'number_delo_il',
                    ...
                    },
                    {
                        ...
                        headerTooltip: 'Первоначальный Взыскатель',
                    ...
                    {
                        headerName: 'Дата цессии',
                        headerTooltip: 'Дата цессии',
                        tooltipField: 'date_recover',
                        field: 'date_recover',
                        filter: true,
                        hide:false,
                        width: 180,
                    ...
                    },
                  {
                    ...
                    headerTooltip: 'Остаток долга',
                    tooltipField: 'ocs_sum',
                    field: 'ocs_sum',
                    filter: true,
                    hide:false,
                    width: 200,
                  ...
                  {
                    headerName: 'Сумма долга по цессии',
                    headerTooltip: 'Сумма долга по цессии',
                    tooltipField: 'sum_cession',
                    field: 'sum_cession',
                    filter: true,
                    hide:false,
                    width: 200,
        ...
        methods: {
            ...
            startDownload(){
              ...
              axios.post(r("taskTemp.update"), {
                params: {
                  method: 'generate',
                  param:JSON.stringify(this.User.pag.credit),
                  name:'credtis'
                }
              }).then((response) => {
                if(response.data.result){
            ...
            clearField(){
              ...
              localStorage.setItem("credit_columnDefs", JSON.stringify(a))
        ...
        mounted () {
          ...
          console.log(this.columnDefsclear)
            ...
            this.getDataUser().then((response)=>{
              if (localStorage.getItem('credit_columnDefs')) {
                this.columnDefs = JSON.parse(localStorage.getItem("credit_columnDefs"));
              }
                this.loadCreditPag()
